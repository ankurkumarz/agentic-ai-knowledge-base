...
- **AI Coding Agents**: Claude Code, OpenAI Codex, Kiro (AWS's ground-up replacement for Amazon Q Developer), Cursor, Devin, GitHub Copilot, and more; **Google Antigravity** (CLI/2.0/SDK/IDE) supersedes Gemini CLI (sunsetting Jun 2026 for Free/Pro/Ultra tiers); **Snowflake CoCo** (formerly Cortex Code) and **Databricks Genie Code** bring data-native and lakehouse-native coding agents into the comparison
- **FreePHDLabor**: Open-source multiagent framework (smolagents-based) automating the scientific research lifecycle via a ManagerAgent orchestrating Ideation, Experimentation, Writeup, Reviewer, and Proofreading agents
...
- **LLM Evaluation Frameworks**: DeepEval, MLFlow, RAGAS, and OpenEvals; plus field-tested methodology for [designing agent evaluations](EvaluationFrameworks/agent-evals-design.md) — grader types (code/model/human), capability vs regression evals, pass@k/pass^k non-determinism metrics, and per-agent-type design patterns (Anthropic, 2026)
...
- **Agent Benchmarks**: METR, Terminal Bench, VisualWebArena, GAIA, BrowseComp (open-web needle-in-haystack), τ2-bench (adversarial multi-turn customer service), and DeepResearch Bench (RACE/FACT evaluation of deep research agents); Terminal-Bench 2.0/2.1 now run via the open-source [Harbor](EvaluationFrameworks/platforms.md#harbor) evaluation harness
...
- **Prompt Hacking Education**: Learn Prompt Hacking — open-source course covering jailbreaks, prompt injection against GPT Assistants/custom GPTs, and red team/blue team LLM defenses
