...
- **Agent Plugins**: Vendor-neutral packaging specification (v1.0.0, August 2026) from a TSC of Amazon, Cursor, Microsoft, OpenAI, Vercel, and Google — bundles Agent Skills (`skills/*/SKILL.md`) and MCP servers (`mcp.json`) into one portable plugin directory with a closed `plugin.json` manifest, fixed-location discovery, path containment, and graduated failure boundaries; marketplaces, install mechanisms, and permission models are explicitly out of scope
