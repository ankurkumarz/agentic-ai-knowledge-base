...
| Demystifying Evals for AI Agents | Field-tested methodology for designing agent evaluations: grader types (code/model/human), capability vs regression evals, pass@k/pass^k metrics, per-agent-type patterns (coding, conversational, research, computer-use), and eval lifecycle roadmap | [EvaluationFrameworks/agent-evals-design.md](../EvaluationFrameworks/agent-evals-design.md) |
...
| Agent Client Protocol (ACP) | Claude Code integrates with ACP-compatible editors (e.g., Zed, JetBrains) via an adapter rather than native ACP support | [Standards/agent-client-protocol.md](../Standards/agent-client-protocol.md) |
