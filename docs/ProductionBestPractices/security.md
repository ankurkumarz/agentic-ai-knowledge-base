...
| Plugin packaging carries no trust guarantees | The Agent Plugins v1.0.0 spec standardizes how skills and MCP servers are bundled, but explicitly defers permissions, signing/provenance, secret handling, enterprise allow/blocklists, and audit-event schemas to future versions | Assumed a standardized package format implied a vetted one; installing a conformant plugin still grants unsigned skill instructions and an MCP server with no declared permission scope | Treat spec conformance as a portability property, not a security property; scan bundled `skills/*/SKILL.md` before install (Cisco Skill Scanner, NVIDIA SkillSpector), pin and review plugin `version`, supply MCP credentials via client-mediated injection rather than `env` literals in `mcp.json`, and enforce allowlists at the client/platform layer until the spec covers them |
...
- [Agent Plugins Specification](../Standards/agent-plugins.md) — packaging standard whose deferred-features list (permissions, provenance, secrets, enterprise controls, audit trails) doubles as a plugin supply-chain risk register
